...
W, H = letter
OUTPUT = r"C:\Users\Mark\Jobsearch\Tailored_Resumes\Mark_Izrailev_Resume_Truewerk_DemandPlanner.pdf"
TEMP   = r"C:\Users\Mark\Jobsearch\Tailored_Resumes\_temp.pdf"

def build(path, f):
    """
    Build resume scaled by f.
    f=1.0 = confirmed tight base that fits on 1 page.
    Larger f = bigger fonts/spacing = more readable.
    """
    # Base values at f=1.0 (tight but confirmed 1-page)
    BODY   = 8.0  * f
    LEAD   = 10.0 * f
    NAME   = 20.0 * f
    SEC    = 9.5  * f
    JOB    = 8.5  * f
    LM = RM = 0.50 * inch          # margins stay fixed so content width stays constant
    TM = BM = (0.42 * f) * inch    # only vertical margins scale
    CW = W - LM - RM               # fixed usable width
    ...
    def section(title):
        return [Paragraph(title, S_section), hr()]
    ...
    story += [
        ...
        hr(before=0),
        Paragraph('DEMAND PLANNER | FORECASTING &amp; S&amp;OP LEADER', S_title),
        Paragraph(
    ...
    # Count pages via canvas callback
    page_nums = []
...
# Binary search: largest f such that page count == 1
lo, hi = 1.0, 1.40
for i in range(14):
    mid = (lo + hi) / 2
    pages = build(TEMP, mid)
    print(f"  f={mid:.4f}  pages={pages}")
    if pages == 1:
        lo = mid
    else:
        hi = mid
